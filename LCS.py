...
'''
This code functions the same as lcs_finder2.cpp (Sent by Max on 8th April 2022)
This isn't the same as process entropy which 
'''
...
class LCSFinder:
    ...
    def FindLCP(self, i, j):
        ...
        while ((pid >= 0) and (i < s_size) and (j < s_size)):
            # print((i + p2 <= s_size),(j + p2 <= s_size),(self.prank[pid][i]) == (self.prank[pid][j]), self.prank[pid][i],self.prank[pid][j])
    ...
    def ComputeAllLCSs(self, inds):
        ...
        for ind in inds:
            ...
            if it < len(sorted_active):
                ...
                print(self.suff_arr[sorted_active[it]], s1idx, lcs)
        ...
        #     while it < len(active):
        #         match = self.FindLCP(self.suff_arr[sorted(active)[it]], s1idx)

        #         if self.suff_arr[sorted(active)[it]]+match > s2idx:
        #             match = s2idx-self.suff_arr[sorted(active)[it]]
        #         else:
        #             lcs = max(lcs, match)
        #             break
        #         lcs = max(lcs, match)
    ...
    def BuildSuffixArray(self):
        ...
        # turn everything back to integers
